...
```css
https://pbbtfwmydxmxdibmtbqq.supabase.co:80/
```
...
```css
pessoas-rest-java/
 ├─ src/main/java/com/example/pessoas/
...
```cmd
echo criando diretório raiz
mkdir pessoas-rest-java
cd pessoas-rest-java
```
...
```cmd
mkdir src\main\java\com\example\pessoas\
```
Criar o subdiretórios com instrução de montagem do pacote: "resources"

```cmd
echo criando pacote java.com.example.pessoas no windows
...
{
    @Query("SELECT p FROM Pessoa p WHERE LOWER(p.nome) LIKE LOWER(CONCAT('%', :termo, '%')) " +
           "OR LOWER(p.cpf) LIKE LOWER(CONCAT('%', :termo, '%'))")
    List<Pessoa> searchByNomeOrCpf(String termo);
}
...
@CrossOrigin(origins = "*")   // habilita CORS para o front-end HTML
public class PessoaController {
    ...
    // GET /pessoas?q=termo
    @GetMapping
    public List<Pessoa> listar(@RequestParam(required = false) String q) {
        if (q == null || q.isEmpty()) {
            return repo.findAll();
        }
    ...
    // PUT /pessoas/{cpf}
    @PutMapping("/{cpf}")
    public ResponseEntity<Pessoa> atualizar(@PathVariable String cpf, @RequestBody Pessoa p) {
...
<project xmlns="http://maven.apache.org/POM/4.0.0"
  ...
  <artifactId>pessoas-rest-java</artifactId>
  <version>0.0.1-SNAPSHOT</version>
  <name>pessoas-rest-java</name>
  <description>Servidor REST de Pessoas em Java</description>
...
<head>
  ...
  <style>
    ...
    img { max-width: 120px; max-height: 120px; margin-top: 5px; }
    table { border-collapse: collapse; width: 100%; margin-top: 20px; }
    th, td { border: 1px solid #ccc; padding: 6px; text-align: left; }
  </style>
</head>
...
async function alterar() {
  ...
  if (!cpf) { alert('CPF é obrigatório.'); return; }
  const dados = coletarDados();
  const resp = await fetch(`${API}/${encodeURIComponent(cpf)}`, {
    method: 'PUT',
    headers: {'Content-Type': 'application/json'},
    body: JSON.stringify(dados)
  });
...
async function remover() {
  ...
  const resp = await fetch(`${API}/${encodeURIComponent(cpf)}`, { method: 'DELETE' });
...
function mostrarResultados(pessoas) 
{
  ...
  pessoas.forEach(p => {
    ...
    tr.innerHTML = `
      ...
      <td>${p.foto ? `<img src="${p.foto}" width="60">` : ''}</td>
    `;
    tbody.appendChild(tr);
  });
}
