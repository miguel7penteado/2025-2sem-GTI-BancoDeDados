...
|-----------------|-----------------|----------------------|-----------------|
| **C**reate | `POST` | `INSERT` | Inserir novos registros. |
| **R**ead | `GET` | `SELECT` | Ler (listar ou buscar) registros existentes. |
| **U**pdate | `PUT` / `PATCH` | `UPDATE` | Atualizar dados de registros existentes. |
| **D**elete | `DELETE` | `DELETE` | Remover registros. |
...
```{plantuml aula-05-Exemplo-01-Modelo_Fisico, plantuml.path="images"}
@startuml
title Modelo Físico-Relacional — Tabela PESSOAS
left to right direction
...
```{plantuml Diagrama-Aplicacao-CRUD, plantuml.path="images"}
 
@startuml
title Arquitetura da Aplicação CRUD — Pessoas (Localhost)
...
' ===================== CONTEXTO (APENHAS LOCALHOST) =====================
node "Máquina Local\n(localhost)" as LOCAL {

  ' ---------- CLIENTE ----------
  node "Navegador Web" as BROWSER {
    component "Cliente Web\nHTML + JavaScript" as CLIENTE <<Front-End>>
  }
  
  ' ---------- SERVIDOR DE APLICAÇÃO ----------
  node "Servidor de Aplicação\nPython 3 (Flask/FastAPI equivalente)" as APP {
    component "API REST\n/pessoas" as API <<REST>>
  }

  ' ---------- SERVIDOR DE BANCO ----------
  database "MySQL 8" as MYSQL {
    rectangle "Esquema: **pessoasdb**" as ESQ
    rectangle "Tabela: **pessoas**\n(cpf PK, nome, endereco,\n data_nascimento, foto)" as TBL
    ESQ -down- TBL
  }
}
...
note bottom of API
  ...
  Operações CRUD:
  • POST   /pessoas        (Create)
  • GET    /pessoas[?q=]   (Read lista)
  • GET    /pessoas/{cpf}  (Read 1)
  • PUT    /pessoas/{cpf}  (Update)
  • DELETE /pessoas/{cpf}  (Delete)
end note

note right of MYSQL
  Usuário de BD: **pessoas_user**
  Host permitido: **localhost**
  Porta: 3306
end note

' ===================== CONEXÕES / PROTOCOLOS =====================
CLIENTE -[#1F78B4]-> API : HTTP/1.1\nRESTful\n**JSON**
API -[#33A02C]-> MYSQL : TCP 3306\nSQL\nuser=pessoas_user\nschema=pessoasdb

' ===================== REGRAS DE ACESSO =====================
note as RESTRACAO <<Constraint>>
  **Restrição de Acesso**
  
  • Acesso permitido apenas a partir da
    **máquina local (localhost)**.
  • Servidor de aplicação e SGBD residem
    na mesma máquina.
end note
RESTRACAO -[dotted]-> LOCAL

' ===================== LEGENDA =====================
legend right
  **Tecnologias**
  - SGBD: MySQL 8
  - App Server: Python 3 (Flask/WSGI)
  - Cliente: HTML + JavaScript (fetch)
  - API: RESTful (JSON)
end legend
...
```{plantuml Cliente-CRUD-Pessoas-01, plantuml.path="images"}

@startuml
title Diagrama de Classes — Cliente Web (HTML + JavaScript, fetch, CRUD Pessoas)
left to right direction
...
class APIService <<service>> {
  - baseUrl : string
  + APIService(baseUrl: string)
  + inserir(pessoa: PessoaDTO) : Promise<Response>
  + alterar(cpf: string, dados: PessoaDTO) : Promise<Response>
  + remover(cpf: string) : Promise<Response>
  + listar() : Promise<Response>
  + pesquisar(termo: string) : Promise<Response>
...
' Utilitários
'========================
class FormUtils <<util>> {
  + validaCampos(ids: string[]) : boolean
  + getFotoBase64(inputId: string) : Promise<string>
  + coletaDados(ids: map) : PessoaDTO
...
class PessoasController <<controller>> {
  - api : APIService
  - utils : FormUtils
  - tabela : TableRenderer
  + PessoasController(api: APIService, utils: FormUtils, tabela: TableRenderer)
  + onInserir() : Promise<void>
  + onAlterar() : Promise<void>
  + onRemover() : Promise<void>
  + onPesquisar() : Promise<void>
  + onListar() : Promise<void>
}

class TableRenderer <<view>> {
  - tbodySelector : string
  + TableRenderer(tbodySelector: string)
  + render(lista: PessoaDTO[]) : void
  + bindRowClick(p: PessoaDTO) : void
}
...
class DOMIds <<config>> {
  ...
  {static} + DATA_NASC : "data_nascimento"
  {static} + FOTO : "foto"
  {static} + PESQUISA : "pesquisa"
  {static} + TBODY : "#tabela"
  {static} + BTN_INSERIR : "btnInserir"
  {static} + BTN_ALTERAR : "btnAlterar"
  {static} + BTN_REMOVER : "btnRemover"
  {static} + BTN_PESQUISAR : "btnPesquisar"
}

'========================
' Relações
'========================
PessoasController --> APIService : usa
PessoasController --> FormUtils : usa
PessoasController --> TableRenderer : atualiza
TableRenderer ..> PessoaDTO
APIService ..> PessoaDTO
FormUtils ..> PessoaDTO
PessoasController ..> DOMIds : consulta ids

note right of PessoasController
Responsável por orquestrar as ações da UI:
• onInserir / onAlterar / onRemover
• onPesquisar / onListar
• Interage com fetch via APIService
• Lê/escreve o DOM via FormUtils e TableRenderer
end note

note bottom of APIService
Encapsula chamadas fetch():
• POST /pessoas
• PUT  /pessoas/{cpf}
• DELETE /pessoas/{cpf}
• GET  /pessoas[?q=]
Inclui readJsonSafe() para lidar com 204/sem corpo.
end note

note bottom of FormUtils
Funções utilitárias da tela:
• validaCampos(['cpf','nome','endereco','data_nascimento'])
• getFotoBase64('foto')
• coletaDados(...) → PessoaDTO
• setBusy(true/false) desabilita botões
end note

note bottom of TableRenderer
Renderiza tabela #tabela:
• render(lista)
• bindRowClick(p) → preenche formulário ao clicar na linha
end note
